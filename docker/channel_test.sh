...
CORE_PEER_GOSSIP_IGNORESECURITY=true CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/fabric/msp/sampleconfig CORE_PEER_COMMITTER_LEDGER_ORDERER=orderer:7050 peer channel create -c medical -a anchorPeer.txt >>log.txt 2>&1
cat log.txt
...
do
echo "###################################### Joining peer$i"
CORE_PEER_COMMITTER_LEDGER_ORDERER=orderer:7050 CORE_PEER_ADDRESS=peer$i:7051 peer channel join -b medical.block >>log.txt 2>&1
cat log.txt
echo '-------------------------------------------------'
...
echo "Peer0 , Peer1 and Peer2 are added to the channel medical"
cat log.txt
exit 0
